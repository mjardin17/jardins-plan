...
export class CompetitorRepository {
  public static async findByBusinessId(businessId: string, passedTx?: TenantTransaction): Promise<CompetitorData[]> {
    try {
      ...
      if (passedTx) {
        return await query(passedTx);
      }
      return await withTenantContext(businessId, query);
    } catch (err) {
  ...
  public static async create(competitor: Omit<CompetitorData, "id">, passedTx?: TenantTransaction): Promise<CompetitorData> {
    ...
    if (passedTx) {
      return await doInsert(passedTx);
    }
    return await withTenantContext(competitor.businessId, doInsert);
  }

  public static async seedDefaultsIfEmpty(businessId: string, passedTx?: TenantTransaction): Promise<CompetitorData[]> {
    const seedOp = async (tx: TenantTransaction) => {
      const existing = await this.findByBusinessId(businessId, tx);
      ...
      for (const def of defaults) {
        const created = await this.create(def, tx);
        seeded.push(created);
      }
    ...
    if (passedTx) {
      return await seedOp(passedTx);
    }
    return await withTenantContext(businessId, seedOp);
  }
}
